...
export const Chatter = ({ onShowToast }) => {
  ...
  const handlePost = async () => {
    ...
    try {
      ...
      onShowToast(err.message || "Failed to create post.", "error");
    }
  };
  ...
  const handleComment = async (postId) => {
    ...
    try {
      ...
      onShowToast(err.message || "Failed to add comment.", "error");
    }
  };
          ...
          <div className="card" style={{ marginBottom: "24px" }}>
            <div style={{ display: "flex", gap: "12px", marginBottom: "16px" }}>
              ...
              <div style={{ flex: 1 }}>
                ...
                />
                <button onClick={handlePost} className="btn btn-primary" disabled={!newPost.trim()}>
                  <Send size={18} />
                  Post
                </button>
              </div>
                    ...
                    <div style={{ flex: 1 }}>
                      ...
                      {post.comments.length > 0 && (
                        ...
                        >
                          {post.comments.map((comment) => {
                            const commentUser = state.users.find((item) => item.userId === comment.userId);
                            return (
                              <div
                      ...
                      <div style={{ display: "flex", gap: "8px", marginTop: "12px" }}>
                        ...
                        <input
                          ...
                          onChange={(event) =>
                            setCommentInputs({ ...commentInputs, [post.postId]: event.target.value })
                          }
                          onKeyDown={(event) => event.key === "Enter" && handleComment(post.postId)}
                  ...
                  return (
                    <div key={user.userId} style={{ display: "flex", gap: "12px", alignItems: "center" }}>
                      ...
                      <div style={{ flex: 1 }}>
                        ...
                        <div style={{ fontSize: "12px", color: "var(--text-secondary)" }}>{user.title}</div>
                      </div>
                      <button
