...
Cookie-scoped CloudCRM-style CRM playground built with FastAPI (backend) and Vite + React (frontend). Each visitor gets an isolated state slice tracked via a cookie; state can be read, replaced, patched, or reset through well-documented APIs and a fully featured UI.
...
- Initial state includes seeded CRM data (users, leads, accounts, opportunities, cases, activities, chatter, and files).
- File uploads are stored on the backend and referenced by URL under `data.files`.
- Auto-generated OpenAPI docs at `/api/docs` and curated `API.md`.
- Frontend panel to inspect/update state and view request/system context.
